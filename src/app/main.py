"""
Legal Debate System - Streamlit Web Application UI

This module contains the Streamlit UI for the legal debate system.
Business logic is imported from src.core modules.
"""
...
def calculate_ml_confidence_score(argument: LegalArgument, state: DebateState = None) -> float:
    ...
    # Combine all factors
    ml_confidence = (
        sentiment_confidence + citation_confidence + thoroughness_confidence + weakness_confidence
    )
...
class LegalRAGSystem:
    ...
    def retrieve_relevant_cases(self, query: str, k: int = 5) -> List[str]:
        ...
        return [
            "Miranda v. Arizona (1966): Suspects must be informed of rights "
            "before interrogation.",
            "Gideon v. Wainwright (1963): Right to legal counsel in criminal "
            "cases.",
            "Terry v. Ohio (1968): Police may conduct stop-and-frisk if "
...
def create_legal_debate_graph(log_container=None):
    ...
    def prosecution_node(state: DebateState) -> DebateState:
        ...
        log_message(
            f"**Round {state['debate_round']}: Prosecution is preparing arguments...**", "🔴"
        )
        ...
        log_message(
            f"Prosecution argument completed. Confidence: {argument.confidence_score:.2f}", "✅"
        )
        return state
    ...
    def defense_node(state: DebateState) -> DebateState:
        ...
        log_message(
            f"**Round {state['debate_round']}: Defense is preparing counter-arguments...**", "🔵"
        )
        ...
        log_message(
            f"Defense argument completed. Confidence: {argument.confidence_score:.2f}", "✅"
        )
        return state
    ...
    def moderator_node(state: DebateState) -> DebateState:
        ...
        log_message(
            f"**Round {state['debate_round']}: Moderator is evaluating "
            "arguments...**",
            "⚖️",
        )
        ...
        log_message(
            f"Moderator analysis completed for Round {state['debate_round']}", "✅"
        )
        return state
...
"""
    ...
    workflow.add_conditional_edges(
        "moderator", should_continue, {"continue": "increment_round", "end": "final_judgement"}
    )
...
def display_argument(argument: LegalArgument, role: str, round_num: int):
    ...
    with st.expander("📊 ML Confidence Analysis"):
        ...
        with col1:
            st.metric(
                "Sentiment Polarity",
                f"{breakdown['sentiment_polarity']:.2f}",
                help="Ranges from -1 (negative) to +1 (positive)",
            )
            st.metric(
                "Citation Count", breakdown["citation_count"], help="Number of case law citations"
            )
            st.metric(
                "Reasoning Length",
                f"{breakdown['reasoning_length']} words",
                help="Length of legal reasoning",
            )
        with col2:
            st.metric(
                "Subjectivity",
                f"{breakdown['sentiment_subjectivity']:.2f}",
                help="0 = objective, 1 = subjective",
            )
            st.metric("Statute Count", breakdown["statute_count"], help="Number of statutes cited")
            st.metric(
                "Weaknesses", breakdown["weaknesses_count"], help="Acknowledged counterarguments"
            )
...
def render_sidebar():
    """Render the sidebar with info and history"""
    with st.sidebar:
        st.header("📖 About")
        st.markdown(
            """
        ...
        The debate runs for up to 3 rounds, with each agent presenting
        arguments based on case law and legal reasoning.
        """
        )
        ...
        if st.session_state.debate_history:
            history_expander = st.expander(
                f"View {len(st.session_state.debate_history)} previous "
                "debate(s)",
                expanded=False,
            )
            with history_expander:
                for idx, history_item in enumerate(reversed(st.session_state.debate_history), 1):
                    ...
                    st.caption(
                        f"Rounds: {history_item['rounds']} | Date: {history_item['timestamp']}"
                    )
...
def main():
    ...
    # Setup page config first (must be first Streamlit command)
    setup_page_config()
    ...
    # Header
    st.markdown(
        '<p class="main-header">⚖️ Legal Debate System</p>',
        unsafe_allow_html=True,
    )
    st.markdown(
        '<p class="sub-header">AI-Powered Multi-Agent Legal Debate '
        "using Google Gemini</p>",
        unsafe_allow_html=True,
    )
    ...
    default_case = (
        "John Doe was arrested for theft after being stopped by police "
        "officers who noticed him acting suspiciously near a jewelry store. "
        "During the stop, officers found stolen jewelry in his backpack. "
        "However, John claims:\n"
        ...
        "Legal Question: Should the evidence (stolen jewelry) be admissible "
        "in court?"
    )
    ...
    if run_button:
        ...
        try:
            ...
            max_rounds = max(
                len(final_state["prosecution_arguments"]), len(final_state["defense_arguments"])
            )
            ...
            for round_num in range(max_rounds):
                ...
                with col2:
                    if round_num < len(final_state["defense_arguments"]):
                        display_argument(
                            final_state["defense_arguments"][round_num], "defense", round_num + 1
                        )
    ...
    if st.session_state.debate_history and not run_button:
        ...
        for idx, history_item in enumerate(
            reversed(st.session_state.debate_history[-3:]), 1
        ):  # Show last 3
            with st.expander(
                f"Debate #{len(st.session_state.debate_history) - idx + 1} - "
                f"{history_item['timestamp']}",
                expanded=False,
            ):
                ...
                if st.button("View Full Debate", key=f"full_view_{idx}"):
